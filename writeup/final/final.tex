...
We show in \ref{fig:transfer-training} the effectiveness of using pre-trained weights from FlappyBird on the game PixelCopter. From the six runs we ran, we found that transfer learning on average had no major impact on the average number of steps per episode. However, the average maximum step per test run was higher using our pre-trained weights. (move Figure 3 out of analysis and into here) hline
...
Flappy bird average number of steps over 15 tests: 14,177.4


\subsection{Analysis}
Our preliminary results showcase that the vanilla DQN model trained on PixelCopter did not generally perform well, even with additional hyperparameter tuning and transfer learning. We decided to increase the number of iterations from 30,000 to 150,000 to see if performance would improve over time. Figure 4 here? Additionally, our preliminary runs involved using transfer learning on solely dense layers. Attempting to combat this issue, we decided to remove x. We ran a total of 40 test runs with 50 episodes per test: 20 of which involved no transfer learning, 10 included transfer learning, and the last 10 included transfer learning with frozen weights.  (y: insert graph and table of averages here). As seen in Figure y, transfer learning with and without frozen weights had no immediate effect on average steps per period.  Additionally, the preliminary results we found indicating a higher average maximum step/reward was not prevalent, as our additional runs seems to combat this finding. 
