...
class Pixelcopter(PyGameWrapper):
    ...
    def getGameState(self):
        ...
        next_terrain_dist = sys.maxsize
        next_terrain = None
        for t in self.terrain_group:
            dist = t.pos.x - current_terrain.pos.x
            if dist > 0 and dist < next_terrain_dist:
                next_terrain_dist = dist
                next_terrain = t
        assert(next_terrain), "next_terrain is undefined"
        
        next_next_terrain_dist = sys.maxsize
        next_next_terrain = None
        for t in self.terrain_group:
            dist = t.pos.x - next_terrain.pos.x
            if dist > 0 and dist < next_next_terrain_dist:
                next_next_terrain_dist = dist
                next_next_terrain = t
        assert(next_next_terrain), "next_next_terrain is undefined"
        
        '''
        this is the old representation of the state
        state = {
        ...
        # and this is the new. we had to add all these in so that our agent could train effectively
        state = {
            "player_y": self.player.pos.y,
            "player_vel": self.player.momentum,  
            # next floor and ceil
            "next_dist": 1,
            "next_ceil_dist": next_terrain.pos.y + self.height * 0.25,
            "next_floor_dist": next_terrain.pos.y - self.height * 0.25,
            # next_next floor and ceil
            "next_next_dist": 2,
            "next_next_ceil_dist": next_next_terrain.pos.y + self.height * 0.25,
            "next_next_floor_dist": next_next_terrain.pos.y - self.height * 0.25,
            # block
            "next_gate_dist_to_player": min_dist,
            "next_gate_block_top": min_block.pos.y,
            "next_gate_block_bottom": min_block.pos.y + min_block.height
